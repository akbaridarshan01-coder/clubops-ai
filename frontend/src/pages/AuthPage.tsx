...
export const AuthPage: React.FC = () => {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      await verifyOtpAndLogin(formData.email, code, 'EMAIL_VERIFY');
  ...
  const handleResendOtp = async () => {
    ...
    setLoading(true);
    try {
      const res: any = await api.sendLoginOtp(formData.email);
              ...
              {step === 2 && (
                <form onSubmit={handleAccountSubmit} className="space-y-4 animate-fade-in">
                  ...
                  <div className="pt-2 flex items-center space-x-2 text-[11px] text-emerald-400">
                    ...
                    <span>A 6-digit OTP will be sent to your email via Gmail SMTP</span>
                  </div>
                ...
                <div className="space-y-6 text-center animate-fade-in">
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-1">
                      Sent to <strong className="text-slate-200">{formData.email}</strong>
                    </p>
                  </div>
