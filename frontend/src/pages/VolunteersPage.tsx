...
export const VolunteersPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-[#EAEFF7]">
        <div>
          ...
          <p className="text-xs text-[#7A829D] mt-0.5">
            Volunteer directory, skill profiles, team rosters and AI workload matcher.
          </p>
        </div>
        ...
        <div className="flex items-center space-x-3">
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl bg-primary-600 hover:bg-primary-500 text-white text-xs font-bold shadow-sm transition-all flex items-center space-x-1.5 whitespace-nowrap active:scale-95"
          >
      ...
      <div className="meet-card p-5 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredVolunteers.map((vol) => {
            ...
            return (
              <div
                key={vol.id}
                className="p-5 rounded-2xl bg-white border border-[#EAEFF7] hover:shadow-sm hover:border-primary-200 transition-all space-y-3 flex flex-col justify-between shadow-2xs"
              >
                <div>
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      <h3 className="text-sm font-bold text-[#191E35] truncate">{vol.name}</h3>
                      <div className="text-xs text-[#7A829D] truncate">{vol.email}</div>
                      {vol.phone && <div className="text-[10px] text-[#9AA1B9] mt-0.5">{vol.phone}</div>}
                    </div>
                    <div className="flex items-center space-x-1.5 flex-shrink-0">
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                        vol.currentWorkload === 'LOW' ? 'bg-emerald-50 text-emerald-700' :
                        vol.currentWorkload === 'MEDIUM' ? 'bg-blue-50 text-blue-700' :
                        vol.currentWorkload === 'HIGH' ? 'bg-amber-50 text-amber-700' : 'bg-rose-50 text-rose-700'
                      }`}>
                        {vol.currentWorkload}
                      </span>
                      <button
                        ...
                        className="p-1.5 rounded-lg text-[#7A829D] hover:text-primary-600 hover:bg-[#F5F6FC] transition-colors"
                      >
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-lg text-[#7A829D] hover:text-rose-600 hover:bg-rose-50 transition-colors disabled:opacity-50"
                      >
                        {deletingId === vol.id ? (
                          <Loader2 className="w-3.5 h-3.5 animate-spin" />
                        ) : (
                  ...
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    {skillsList.length > 0 ? (
                      skillsList.slice(0, 5).map((s, idx) => (
                        <span
                          key={idx}
                          className="text-[10px] px-2 py-0.5 rounded-md bg-[#F5F6FC] text-[#48506E] border border-[#E4E8F2] font-medium"
                        >
                          {s}
                        </span>
                      ))
                    ) : (
                      <span className="text-[10px] text-[#8C93AE] italic">No specific skills listed</span>
                    )}
                  </div>
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="mt-2.5 pt-2.5 border-t border-[#F0F2F9] space-y-1.5">
                        <span className="text-[10px] font-bold text-[#7A829D] uppercase tracking-wider block">
                          Assigned Tasks ({volTasks.length}):
                        </span>
                        <div className="space-y-1">
                          {volTasks.map(t => (
                            <div key={t.id} className="flex items-center justify-between text-[11px] bg-[#F8F9FE] border border-[#E9EDF7] rounded-lg px-2.5 py-1 text-[#323955]">
                              ...
                              <button
                                ...
                                className="text-rose-600 hover:text-rose-700 transition-colors flex items-center space-x-1 text-[10px] bg-rose-50 hover:bg-rose-100 px-1.5 py-0.5 rounded border border-rose-200 disabled:opacity-50 shrink-0 font-medium"
                              >
                ...
                <div className="pt-3 border-t border-[#F0F2F9] flex items-center justify-between text-[11px] text-[#7A829D]">
                  <div className="flex items-center space-x-1.5 truncate">
                    <span className="truncate">Team: <strong className="text-[#191E35]">{vol.team?.name || 'General Operations'}</strong></span>
                    {vol.experienceYears !== undefined && vol.experienceYears !== null && (
                      <span className="text-[#8C93AE]">• {vol.experienceYears}y exp</span>
                    )}
                  </div>
