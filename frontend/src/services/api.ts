...
export const api = {
  ...
  executeAllMeetingActions: (meetingId: string) => request(`/meetings/${meetingId}/execute-all-actions`, { method: 'POST' }),

  // Decisions
  getDecisions: (eventId: string) => request(`/decisions?eventId=${eventId}`),
  createDecision: (data: any) => request('/decisions', { method: 'POST', body: JSON.stringify(data) }),
  updateDecision: (id: string, data: any) => request(`/decisions/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
  deleteDecision: (id: string) => request(`/decisions/${id}`, { method: 'DELETE' }),
