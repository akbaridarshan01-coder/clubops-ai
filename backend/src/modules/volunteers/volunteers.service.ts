...
export class VolunteersService {
  async getVolunteers(clubId: string, eventId?: string, teamId?: string) {
    ...
    return await prisma.volunteer.findMany({
      ...
      include: {
        team: true,
        event: { select: { id: true, name: true, type: true, status: true, date: true } },
        skillRecords: true,
      },
  ...
  /**
   * Finds an existing volunteer by name or email in the club.
   * If not found, automatically creates a new volunteer record.
   * Called whenever a person is mentioned in a meeting / assigned a task.
   */
  async ensureVolunteer(clubId: string, data: {
    name: string;
    email?: string;
    eventId?: string;
    skills?: string[];
    phone?: string;
  }): Promise<import('@prisma/client').Volunteer> {
    const email = data.email || `${data.name.toLowerCase().replace(/\s+/g, '.')}@clubops.org`;

    // 1. Try to find by exact name (case-insensitive)
    const byName = await prisma.volunteer.findFirst({
      where: {
        clubId,
    ...
    const byEmail = await prisma.volunteer.findFirst({
      where: { clubId, email },
    });
    if (byEmail) return byEmail;

    // 3. Not found — auto-create as a new volunteer
    console.log(`[VolunteersService] Auto-enrolling new volunteer: ${data.name} (${email})`);
    const skills = data.skills && data.skills.length > 0 ? data.skills : ['General'];
